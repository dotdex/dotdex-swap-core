...
async function adjustPeripherySourceCode(address) {
  const PATH_GET = '../dotdex-swap-periphery/contracts/libraries/PancakeLibraryTemplate.sol'
  const PATH_PUT = '../dotdex-swap-periphery/contracts/libraries/PancakeLibrary.sol'

  const contract = await ethers.getContractAt('DotDexFactory', address)
...
async function main() {
  ...
  DotDexFactory = await deploy('DotDexFactory', [admins._devaddr])
